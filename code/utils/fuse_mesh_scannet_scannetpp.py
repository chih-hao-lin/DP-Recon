...
if not os.path.exists(intrinsic_path):
    intrinsic_path = f'{os.path.dirname(args.input_scalemat)}/camera_intrinsic.npy'
...
mesh = trimesh.load(args.output_mesh)
scale_matrix = np.linalg.inv(scale_mat[:3, :3].T)
mesh.vertices = (mesh.vertices - scale_mat[:3, 3]) @ scale_matrix
out_mesh_path = args.output_mesh.replace('.ply', '_norm.ply')
mesh.export(out_mesh_path)
print(f"Saved normalized mesh to {out_mesh_path}")
