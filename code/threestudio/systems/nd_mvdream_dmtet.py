...
class ND_DMTet_MV(_Base_ND_DMTet_MV):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # save mesh temp
        mesh_raw = self.geometry.isosurface()
        mesh = trimesh.Trimesh(
            vertices=mesh_raw.v_pos.detach().cpu().numpy(),
            faces=mesh_raw.t_pos_idx.detach().cpu().numpy(),
        )
        mesh_name = f"it{self.true_global_step:06d}-{front['index'][0]:04d}.ply"
        mesh_save_path = os.path.join(self.get_save_dir(), mesh_name)
