...
class NVDiffRasterizer(Rasterizer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # mesh = self.geometry.isosurface()             # use input mesh directly
        ...
        # render visibility
        max_trans = self.geometry.get_vis(mesh.v_pos)
        gb_vis, _ = self.ctx.interpolate_one(max_trans, rast, mesh.t_pos_idx)
        vis_map = self.ctx.antialias(gb_vis, rast, v_pos_clip, mesh.t_pos_idx)
        vis_map[vis_map > 1.0] = 1.0            # [0, 1]

        out.update({"vis_map": vis_map})
