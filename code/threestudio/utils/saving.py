...
def fix_mesh(obj_file, save_path):
    print("==============fix mesh==================")
    mesh = trimesh.load(obj_file, validate=True)
...
class SaverMixin:
    ...
    def _save_mtl(
        ...
        mesh_str = os.path.basename(filename).replace(".mtl", "")
        ...
        if map_Kd is not None:
            map_Kd_save_path = os.path.join(
                os.path.dirname(mtl_save_path), f"{mesh_str}_texture_kd.{map_format}"
            )
            mtl_str += f"map_Kd {mesh_str}_texture_kd.{map_format}\n"
        ...
        if map_Ks is not None:
            map_Ks_save_path = os.path.join(
                os.path.dirname(mtl_save_path), f"{mesh_str}_texture_ks.{map_format}"
            )
            mtl_str += f"map_Ks {mesh_str}_texture_ks.{map_format}\n"
            self._save_rgb_image(
                map_Ks_save_path,
                map_Ks,
                data_format="HWC",
        ...
        if map_Bump is not None:
            map_Bump_save_path = os.path.join(
                os.path.dirname(mtl_save_path), f"{mesh_str}_texture_nrm.{map_format}"
            )
            mtl_str += f"map_Bump {mesh_str}_texture_nrm.{map_format}\n"
            self._save_rgb_image(
                map_Bump_save_path,
                map_Bump,
                data_format="HWC",
        ...
        if map_Pm is not None:
            map_Pm_save_path = os.path.join(
                os.path.dirname(mtl_save_path), f"{mesh_str}_texture_metallic.{map_format}"
            )
            mtl_str += f"map_Pm {mesh_str}_texture_metallic.{map_format}\n"
        ...
        if map_Pr is not None:
            map_Pr_save_path = os.path.join(
                os.path.dirname(mtl_save_path), f"{mesh_str}_texture_roughness.{map_format}"
            )
            mtl_str += f"map_Pr {mesh_str}_texture_roughness.{map_format}\n"
            self._save_grayscale_image(
                map_Pr_save_path,
                map_Pr,
                data_range=(0, 1),
