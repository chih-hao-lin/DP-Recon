...
class MonoSDFLoss(nn.Module):
    def __init__(self, rgb_loss, 
        ...
        self.duster_depth_loss = torch.nn.MSELoss(reduction='mean')
    ...
    def get_depth_rank_loss(self, depth_pred, depth_gt, mask, N=10000):

        M = depth_pred.shape[0]
        depth_gt = depth_gt.reshape(M, -1)
        mask = mask.reshape(M, -1)

        valid_indices = torch.where(mask > 0.5)[0]
        valid_M = valid_indices.size(0)

        if valid_M < 2:
            print('depth_rank_loss: valid_M < 2, return 0')
            return torch.tensor(0.0).cuda().float()

        idx1 = valid_indices[torch.randint(0, valid_M, (N,))]
        idx2 = valid_indices[torch.randint(0, valid_M, (N,))]
        idx2 = torch.where(idx1 == idx2, (idx2 + 1) % valid_M, idx2)

        depth_pred1 = depth_pred[idx1]
        depth_pred2 = depth_pred[idx2]
        depth_gt1 = depth_gt[idx1]
        depth_gt2 = depth_gt[idx2]

        gt_rank = (depth_gt1 < depth_gt2).float()
        pred_rank = (depth_pred1 < depth_pred2).float()

        rank_loss = nn.functional.mse_loss(pred_rank, gt_rank)
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        # depth_loss = self.get_depth_rank_loss(depth_pred, depth_gt, mask)
...
class DPReconLoss(MonoSDFLoss):
    def __init__(self, rgb_loss, 
        ...
        print(f"[INFO]: using weight for loss RGB_{self.rgb_weight} EK_{self.eikonal_weight} SM_{self.smooth_weight} Depth_{self.depth_weight} NormalL1_{self.normal_l1_weight} NormalCos_{self.normal_cos_weight}\
    ...
    def forward(self, model_outputs, ground_truth, call_reg=False, call_bg_reg=False):
        ...
        if 'bg_depth_values' in model_outputs:
            if 'bg_mask' in model_outputs:
                ...
                bg_mask = torch.ones_like(model_outputs['bg_mask']).cuda()
            else:
                bg_mask = (ground_truth['segs'] != 0).cuda()            # use gt mask directly
            # background_reg_loss = self.bg_tv_loss(model_outputs['bg_depth_values'], model_outputs['bg_normal_map'], bg_mask)
        ...
        # add sds loss
        if 'loss_rgb_sd' in model_outputs:
            loss_rgb_sd = model_outputs['loss_rgb_sd']
            output['loss_rgb_sd'] = loss_rgb_sd
            output['sds_loss'] = loss_rgb_sd * self.sds_weight
            output['loss'] = output['loss'] + loss_rgb_sd * self.sds_weight
        ...
        output['loss'] = output['loss'] + self.semantic_weight * semantic_loss + self.reg_vio_weight* sample_sdf_loss + self.bg_reg_weight * background_reg_loss + self.bg_surface_obj_reg_weight * bg_surface_obj_reg
        return output